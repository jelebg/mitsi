function adjustPanelSize() {
	...
	// datasourceFilter.style.width = (datasourcePanel.style.width.replace("px",
	// "")-10)+"px";
}
...
function displayDatasource(datasourceName) {
	...
	/*
	 * var imgUnroll = document.createElement("IMG"); imgUnroll.src =
	 * "img/unroll.png"; var aUnroll = document.createElement("A"); aUnroll.href =
	 * "";
	 */
	/*
	 * aUnroll.onclick = function(event) { //alert("unroll:"+divId);
	 * unrollDatasource(this, event, name); return false; };
	 */
	/*
	 * aUnroll.appendChild(imgUnroll); aUnroll.style.position = "absolute";
	 * aUnroll.style.right = "10px";
	 * 
	 * //var imgMenu = document.createElement("IMG"); //imgMenu.src =
	 * "img/unroll.png"; var aMenu = document.createElement("A");
	 * aMenu.textContent = "..."; aMenu.href = ""; aMenu.onclick =
	 * function(event) { showDatasourceMenu(this, event, name);
	 * event.stopPropagation(); return false; }; //aMenu.appendChild(imgMenu);
	 * aMenu.style.position = "absolute"; aMenu.style.right = "40px";
	 */

	datasourceContext.smallMenu = new MitsiPanelSmallMenu(
			datasourceContext.div, "20px", 25, 7, [ {
				title : "unroll",
				imgsrc : "img/unroll.png"
			},
			// { title:"search", imgsrc:"img/search.png" },
			{
				...
				title : "details",
				imgsrc : "img/details.png"
			}, {
				title : "refresh",
				imgsrc : "img/refresh.png"
			}, {
				title : "connect",
				imgsrc : "img/connected-false.png",
				toggleimg : "img/connected-true.png",
				toggled : datasourceContext.connected
			} ]);
	// datasourceSmallMenu.getDiv(1).onclick = function(event) {
	...
	datasourceContext.smallMenu.setOnClick(1, function(event) {
		...
		// try {
		// TODO : à refaire en générique de toute façon, ici ne marchera à cause
		// du scope de la fonction
		datasourcePopupMenu.show(name, datasourceContext.smallMenu.getDiv(1),
				[ {
					label : "menu de " + name,
					title : "title de " + name
				} ]);
	...
	datasourceContext.filter = new DynamicFilter(/* ivUnrolledButtons */td4, "",
			"filter tables/cols here ...", createOnFilterDatasource(name), {
				"table" : {
					label : "table names",
					value : true
				},
				...
				"constraint" : {
					label : "constraint names",
					value : true
				}
			});

	divUnrolledButton1.title = "display/hide tables";
	datasourceContext.toggleTable = new MitsiToggle(true, "img/table.png",
			"16px", divUnrolledButton1, createOnFilterDatasource(name,
					datasourceContext.filter));
	divUnrolledButton2.title = "display/hide views";
	datasourceContext.toggleView = new MitsiToggle(true, "img/view.png",
			"16px", divUnrolledButton2, createOnFilterDatasource(name,
					datasourceContext.filter));
	divUnrolledButton3.title = "display/hide materialized views";
	datasourceContext.toggleMatView = new MitsiToggle(true, "img/matview.png",
			"16px", divUnrolledButton3, createOnFilterDatasource(name,
					datasourceContext.filter));
	...
	datasourceContext.accordion = new MitsiAccordion(
			datasourceContext.divUnrolled, datasourceUnrolledPrefix,
			datasourceContext.smallMenu.getDiv(0), false, true, function() {
	...
	/*
	 * var columns = ["ID", "NAME", "COL1", "COL2", "COL3", "COL4", "COL4",
	 * "COL5", "COL6"]; var tables = ["CLIENTS", "FOURNISSEURS", "EMPLOYES",
	 * "CAMIONS", "PELLES", "RATEAUX"];
	 *  // TODO : à supprimer for(var ti=0; ti!=tables.length; ti++) { var
	 * divTable = document.createElement("DIV"); divTable.className =
	 * "datasourceTable"; divTable.style.width = "100%"; divTable.textContent =
	 * tables[ti];
	 * 
	 * /*var imgColUnroll = document.createElement("IMG"); imgColUnroll.src =
	 * "img/unroll.png"; var aColUnroll = document.createElement("A");
	 * aColUnroll.href = ""; aColUnroll.appendChild(imgColUnroll);
	 * aColUnroll.style.position = "absolute"; aColUnroll.style.right = "10px";
	 * divUnrolled.appendChild(aColUnroll); / var smallMenu = new
	 * MitsiPanelSmallMenu(divTable, "16px", 20, [{ title:"unroll",
	 * imgsrc:"img/unroll.png" }, { title:"search", imgsrc:"img/search.png" }, {
	 * title:"menu", imgsrc:"img/menu.png" }, { title:"details",
	 * imgsrc:"img/details.png" }, { title:"table datas", imgsrc:"img/table.png" }
	 * ]); smallMenu.getDiv(1).onclick = function() { alert("search"); return
	 * false; }; smallMenu.getDiv(2).onclick = function() { alert("menu");
	 * return false; }; smallMenu.getDiv(3).onclick =
	 * createOnClickShowTableDetail("details.table."+name+"."+tables[ti],
	 * smallMenu.getDiv(3), name, tables[ti]); smallMenu.getDiv(4).onclick =
	 * createOnClickShowTable("data.table."+name+"."+tables[ti],
	 * smallMenu.getDiv(3), name, tables[ti]);
	 * 
	 * divAllTables.appendChild(divTable);
	 * 
	 * var divCols = document.createElement("DIV"); divCols.className =
	 * "datasourceColumns"; divCols.style.width = "100%";
	 * 
	 * for(var ci=0; ci!=columns.length; ci++) { var divCol =
	 * document.createElement("DIV"); divCol.className = "datasourceColumn";
	 * divCol.style.width = "100%"; divCol.textContent = tables[ti].substring(0,
	 * 3)+"_"+columns[ci]; divCols.appendChild(divCol); }
	 * divAllTables.appendChild(divCols);
	 * 
	 * new MitsiAccordion(divCols, datasourceColumnsUnrolledPrefix,
	 * //aColUnroll, smallMenu.getDiv(0), false, false);
	 *  }
	 * 
	 * //div.appendChild(aUnroll); //div.appendChild(aMenu);
	 */
...
function displayDatabaseObjects(datasourceName) {
	...
	for (var ti = 0; ti != objects.length; ti++) {
		...
		if (datasource.currentSchema
				&& datasource.currentSchema != table.id.schema) {
			continue;
		}
		...
		// divTable.textContent = table.id.name; // TODO :recopier les autres
		// champs qqpart ...
		 ...
		 * "img/unroll.png"; var aColUnroll = document.createElement("A");
		 * aColUnroll.href = ""; aColUnroll.appendChild(imgColUnroll);
		 * aColUnroll.style.position = "absolute"; aColUnroll.style.right =
		 * "10px"; divUnrolled.appendChild(aColUnroll);
		 */
		var smallMenu = new MitsiPanelSmallMenu(divTable, "16px", 20, 7, [ {
			title : "unroll",
			imgsrc : "img/unroll.png"
		}, {
			title : "links",
			imgsrc : "img/proxgraph.png"
		}, {
			title : "menu",
			imgsrc : "img/menu.png"
		}, {
		...
		// smallMenu.getDiv(1).onclick =
		// createOnClickShowTableLinksInPopup("links.table."+datasourceName+"."+table.id.name,
		// smallMenu.getDiv(1), datasourceName, table.id.schema, table.id.type,
		// table.id.name);
		smallMenu.getDiv(1).onclick = createOnClickShowTableLinks(
				datasourceName, table.id.schema, table.id.type, table.id.name);
		smallMenu.getDiv(2).onclick = function() {
			alert("menu");
			return false;
		};
		smallMenu.getDiv(3).onclick = createOnClickShowTableDetail(
				"details.table." + datasourceName + "." + table.id.name,
				smallMenu.getDiv(3), datasourceName, table.id.schema,
				table.id.type, table.id.name);
		smallMenu.getDiv(4).onclick = createOnClickShowTable("data.table."
				+ datasourceName + "." + table.id.name, smallMenu.getDiv(3),
				datasourceName, table.id.schema, table.id.name);
		...
		for (var ci = 0; ci != columns.length; ci++) {
			...
			divCol.textContent = column.name; // TODO : et les autres
												// propriétés
			table.divColumns.appendChild(divCol);
		}
		divAllTables.appendChild(table.divColumns);

		new MitsiAccordion(table.divColumns, datasourceColumnsUnrolledPrefix,
		// aColUnroll,
		smallMenu.getDiv(0), false, false);
	...
	return function(newvalue, categories) {
		try {
			...
			for (var i = 0; i != datasourceContext.objects.length; i++) {
				...
				// TODO : filtrer aussi sur les colonnes, indexs, partitions,
				// etc.
			}
		} catch (e) {
			console.log(e);
...
function createOnClickShowDatasourceDetail(detailScreenId, div, datasource) {
	return function(event) {
		try {
			detailScreen.show(detailScreenId, "details?datasource="
					+ datasource, div);
			event.stopPropagation();
		} catch (e) {
			console.log(e);
...
function createOnClickShowTableDetail(detailScreenId, div, datasource, owner,
		objectType, objectName) {
	return function(event) {
		try {
			detailScreen.show(detailScreenId, "details?datasource="
					+ datasource + "&type=" + objectType + "&owner=" + owner
					+ "&name=" + objectName, div);
		} catch (e) {
			console.log(e);
		}
...
function createOnClickShowTableLinks(datasourceName, owner, objectType,
		objectName) {
	return function(event) {
		try {
			// var datasource = CONTEXT.getDatasource(datasourceName);
...
function createOnClickShowTableLinksInPopup(detailScreenId, div, datasource,
		owner, objectType, objectName) {
	return function(event) {
		try {
			detailScreen.show(detailScreenId, "links?datasource=" + datasource
					+ "&type=" + objectType + "&owner=" + owner + "&name="
					+ objectName, div);
		} catch (e) {
			console.log(e);
		}
		...
		try {
			if (connected) {
				...
				callGsonServlet("DisconnectServlet", {
					...
					// TODO : que faire si la disconnexion a échoué ? à priori
...
function createOnClickShowTable(detailScreenId, div, datasource, owner, table) {
	return function(event) {
		try {
			detailScreen.show(detailScreenId, "table?datasource=" + datasource
					+ "&owner=" + owner + "&name=" + table, div);
		} catch (e) {
			console.log(e);
		}
		event.stopPropagation();
		...
		if (regex == null) {
			...
			if (datasource.name.match(regex)) {
				display = true;
					...
					&& datasource.description.match(regex)) {
				display = true;
			} else {
				if (datasource.tags) {
...
function bodyOnLoad() {
	...
	hightlightToggle = new MitsiToggle(false,
			"img/toggle_hightlight_3_true.png", null, gid("btnHighlight"), null);
	datasourcePopupMenu = new PopupMenu("datasourcePopupMenu", gid("around"),
			"200px", [ {
				label : "menu 1",
				title : "title 1"
			}, {
	...
	detailScreen = new PopupDetailsScreen("detailsScreen", gid("around"),
			gid("around"), 15, 15);

	ftextSqlInput = new FText(gid("sqlinput"), gid("sqlLineNumber"));

	linksConfiguration = new MitsiLinksConfiguration(
			gid("linksPathConfiguration"));
	...
	vSeparatorFor(gid("vseparator"), [ gid("datasourcePanel") ], [
			gid("rightPanel"), gid("sqlresulttable") ]);
	hSeparatorFor(gid("hseparator"), [ gid("sqlinputGlobal") ], [
			gid("sqlresult"), gid("sqlresulttable") ]);
	...
	/*
	 * addDataSource("MONPROJET-ENV-DE-RECETTE", "Réservé à l'équipe de
	 * recette", ["MonProjet", "Recette", "Attention!!"]);
	 * addDataSource("MONPROJET-ENV-DE-DEV", "Réservé aux devs", ["MonProjet",
	 * "dev"]); addDataSource("MONPROJET-ENV-DE-PREPROD", "Réservé aux équipes
	 * de prod", ["MonProjet", "PreProd", "Attention!!"]);
	 * addDataSource("AUTREPROJET-ENV-DE-RECETTE", "Réservé à l'équipe de
	 * recette", ["AutreProjet", "Recette", "Attention!!"]);
	 */

	callGsonServlet("GetClientStatusServlet", {}, function(response) {
		console.log(response);
		if (response.datasources != null) {
			...
			for (var i = 0; i != response.datasources.length; i++) {
				...
				// addDataSource(datasource.name, datasource.description,
				// datasource.tags, datasourceCtx);
...
function runSQL() {
	...
	callGsonServlet("RawSQLBeginServlet", {
		...
		CONTEXT.handsontable = sqlTableInit(gid("sqlresulttable"),
				response.columns, CONTEXT.handsontableData,
...
function runSQLFetchMore() {
	...
	callGsonServlet("RawSQLFetchServlet", {
		"datasourceName" : datasource,
		"nbRowToFetch" : nbRowsToFetchGlobal
	// TODO : à rendre paramétrable
	}, function(response) {
		console.log(response);

		sqlTableDisplayMore(CONTEXT.handsontable, CONTEXT.handsontableData,
				response.results,
				(response.results.length < nbRowsToFetchGlobal));
...
function changeCurrentSchema(datasourceName) {
	// TODO :est-ce vraiment nécessaire pour la connexion spécifique MITSI ?
	// appeler directement EVENT_DatasourceSchemaChange ?
